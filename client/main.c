...
static void render_multi_choice(void) {
    ...
    /* Buttons */
    draw_button(WINDOW_W / 2 - 150, 450, 300, 60, "CREER UNE PARTIE", COLOR_SUCCESS, 0);
    draw_button(WINDOW_W / 2 - 150, 540, 300, 60, "REJOINDRE", COLOR_BUTTON, 0);
    draw_button(WINDOW_W / 2 - 90, 650, 180, 50, "RETOUR", COLOR_DANGER, 0);
}
...
static void render_lobby(void) {
    ...
    draw_text(font_L, title, WINDOW_W / 2, 80, COLOR_ORANGE);
    
    /* Players header */
    draw_text(font_S, "JOUEURS :", WINDOW_W / 2, 160, COLOR_WHITE);
    ...
    for (i = 0; i < current_lobby.player_count; i++) {
        ...
        draw_text(font_S, current_lobby.players[i], WINDOW_W / 2, 210 + i * 40, color);
        
        /* Show host indicator */
        if (i == 0) {
            draw_text(font_S, "(Host)", WINDOW_W / 2 + 100, 210 + i * 40, COLOR_GREY);
        }
    }
    
    /* Host controls or waiting message */
    if (current_lobby.is_host) {
        draw_button(WINDOW_W / 2 - 150, 500, 300, 60, "LANCER LA PARTIE", COLOR_SUCCESS, 
                    current_lobby.player_count < 2);
        if (current_lobby.player_count < 2) {
            draw_text(font_S, "En attente d'autres joueurs...", WINDOW_W / 2, 450, COLOR_GREY);
        }
    } else {
        draw_text(font_S, "En attente du lancement...", WINDOW_W / 2, 500, COLOR_GREY);
    }
    
    draw_button(WINDOW_W / 2 - 90, 620, 180, 50, "QUITTER", COLOR_DANGER, 0);
    
    /* Kick instruction for host */
    if (current_lobby.is_host && current_lobby.player_count > 1) {
        draw_text(font_S, "(Clic droit pour expulser)", WINDOW_W / 2, 700, COLOR_GREY);
    }
}
...
static void render_game_grid(void) {
    ...
    /* Grid background */
    fill_rect(GRID_OFFSET_X - 5, GRID_OFFSET_Y - 5, 
              GRID_W * BLOCK_SIZE + 10, GRID_H * BLOCK_SIZE + 10, 
              0x151520);
    
    /* Grid lines */
    for (i = 0; i <= GRID_H; i++) {
        fill_rect(GRID_OFFSET_X, GRID_OFFSET_Y + i * BLOCK_SIZE, 
                  GRID_W * BLOCK_SIZE, 1, COLOR_GRID);
    }
    for (j = 0; j <= GRID_W; j++) {
        fill_rect(GRID_OFFSET_X + j * BLOCK_SIZE, GRID_OFFSET_Y, 
    ...
    for (i = 0; i < GRID_H; i++) {
        for (j = 0; j < GRID_W; j++) {
            if (game.grid[i][j] != 0) {
                draw_styled_block(GRID_OFFSET_X + j * BLOCK_SIZE + 1,
                                  GRID_OFFSET_Y + i * BLOCK_SIZE + 1,
                                  BLOCK_SIZE - 2, game.grid[i][j]);
            }
        }
    }
}
...
static void render_solo(void) {
    ...
    draw_text(font_L, score_text, WINDOW_W / 2, 40, COLOR_WHITE);
    ...
    /* Game over message */
    if (game.game_over) {
        fill_rect(0, WINDOW_H / 2 - 60, WINDOW_W, 120, 0x000000AA);
        draw_text(font_L, "GAME OVER!", WINDOW_W / 2, WINDOW_H / 2 - 20, COLOR_DANGER);
        draw_text(font_S, "Cliquez pour revenir au menu", WINDOW_W / 2, WINDOW_H / 2 + 30, COLOR_WHITE);
    }
}
...
static void render_multi_game(void) {
    ...
    /* Pieces - greyed if not our turn */
    render_pieces(!is_my_turn());
    
    /* Dragged piece (only if our turn) */
    if (is_my_turn()) {
        render_dragged_piece();
    }
}
...
static void handle_multi_choice_click(void) {
    ...
    /* Create room */
    if (point_in_rect(mouse_x, mouse_y, WINDOW_W / 2 - 150, 450, 300, 60)) {
        memset(&pkt, 0, sizeof(pkt));
        pkt.type = MSG_CREATE_ROOM;
        net_send(&pkt);
    }
    /* Join room */
    else if (point_in_rect(mouse_x, mouse_y, WINDOW_W / 2 - 150, 540, 300, 60)) {
        memset(input_buffer, 0, sizeof(input_buffer));
        current_state = ST_JOIN_INPUT;
    }
    /* Back button */
    else if (point_in_rect(mouse_x, mouse_y, WINDOW_W / 2 - 90, 650, 180, 50)) {
        net_close();
        current_state = ST_MENU;
    }
}
...
static void handle_lobby_click(void) {
    ...
    if (current_lobby.is_host && current_lobby.player_count >= 2 &&
        point_in_rect(mouse_x, mouse_y, WINDOW_W / 2 - 150, 500, 300, 60)) {
        memset(&pkt, 0, sizeof(pkt));
        pkt.type = MSG_START_GAME;
        net_send(&pkt);
    }
    /* Quit button */
    else if (point_in_rect(mouse_x, mouse_y, WINDOW_W / 2 - 90, 620, 180, 50)) {
        net_close();
        current_state = ST_MENU;
    }
}
            ...
            switch (e.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    if (e.button.button == SDL_BUTTON_LEFT) {
                        ...
                        } else if (current_state == ST_MULTI_GAME) {
